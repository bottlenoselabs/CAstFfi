...
using CAstFfi.Infrastructure.Clang;
using CAstFfi.IO;
using CAstFfi.IO.Input.Sanitized;
using CAstFfi.IO.Output;
using CAstFfi.Native;
using static bottlenoselabs.clang;

namespace CAstFfi.Domain.Explore;
...
public sealed class ExploreContext
{
    ...
    public CTargetPlatform TargetPlatformActual { get; }
        ...
        ExploreInfoNode? parentInfo)
    {
        ...
        var typeNameActual = TypeName(kind, type, parentInfo?.Name);
    ...
    private static (CTargetPlatform TargetPlatform, int PointerWidth) GetTargetPlatform(
        CXTranslationUnit translationUnit)
    {
        ...
        var platform = new CTargetPlatform(platformString);
 ...
 */
        ...
        CKind? parentKind)
    {
        ...
        var up = new InvalidOperationException($"Unknown type kind '{type.kind}'.");
        throw up;
    }
